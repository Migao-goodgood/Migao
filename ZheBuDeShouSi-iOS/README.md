...
“趋势”页的身体趋势使用本地 Vision 识别 InBody 报告，识别结果会先进入可编辑表单，必须由用户核对后才保存。页面只在右上角保留扫描/录入入口，空状态不重复放置按钮，也不使用装饰性大标题。整体采用温暖纸张底、墨蓝文字、雾蓝/鼠尾草/灰粉小面积强调、细线和留白的编辑感布局；这是对参考作品“真实内容 + 极简提炼”的设计转译，不直接复制图片。Hello Kitty 是默认的三维示意形象：使用 SceneKit 程序化几何完整展示耳朵、蝴蝶结、脸部、服装、手臂和双脚，身体参数会随记录变化，干净背景与接触阴影不会遮挡全身。人物、小猫、小狗和自定义照片仍可从“更多形象”进入。它不是医学级人体扫描，也不会从单张照片推断真实三维网格；心情由用户主动记录，阶段评价仅供健康记录参考，不替代医生诊断。报告照片不会上传到第三方服务。

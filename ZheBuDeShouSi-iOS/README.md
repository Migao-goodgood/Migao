...
- 趋势折线图、周期切换、统计和历史记录
- 身体趋势工作区：InBody 报告照片 OCR 草稿、手动录入、时间线、三维示意形象、心情和阶段评价
...
“趋势”页的身体趋势使用本地 Vision 识别 InBody 报告，识别结果会先进入可编辑表单，必须由用户核对后才保存。页面只在右上角保留扫描/录入入口，空状态不重复放置按钮，也不使用装饰性大标题。整体采用温暖纸张底、墨蓝文字、雾蓝/鼠尾草/灰粉小面积强调、细线和留白的编辑感布局；这是对参考作品“真实内容 + 极简提炼”的设计转译，不直接复制图片。人物、小猫、小狗和自定义照片是由 SceneKit 参数驱动的三维示意形象：人物和宠物使用大头小身体的 kawaii 比例、圆润短肢、大眼高光、腮红和小蝴蝶结，配合 PBR 磨砂材质、柔和三点灯光、展台接触阴影和轻微呼吸动画，并支持拖动旋转查看。它不是医学级人体扫描，也不会从单张照片推断真实三维网格；心情由用户主动记录，阶段评价仅供健康记录参考，不替代医生诊断。报告照片不会上传到第三方服务。

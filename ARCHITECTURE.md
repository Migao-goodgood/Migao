...
- `ZheBuDeShouSi-iOS/ZheBuDeShouSi/Shared/UI/WeightCalendarView.swift`: shared `WeightModuleTitle` (compact horizontal 19-point ink heading matching the hierarchy of `最新测量`), local-date grouping, one representative weight per day, month grid presentation, and calendar intensity rules driven by the shared pink-to-blue journey palette; the calendar module has no vertical divider.

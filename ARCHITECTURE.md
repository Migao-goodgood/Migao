...
- `ZheBuDeShouSi-iOS/ZheBuDeShouSi/Shared/Models/Models.swift`: domain enums/models, production-empty `AppState`, injectable `UserDefaults`, optional current/start/goal projections, canonical-kilogram storage, `WeightUnit` 公斤/斤 conversion and legacy kg/g preference migration, invalid persisted-weight filtering, derived progress and color rules, the shared `WeightJourneyPalette`, avatar snapshot persistence, and user actions.
...
- `ZheBuDeShouSi-iOS/ZheBuDeShouSi/Shared/UI/AddMediaActionLabel.swift`: shared interaction-free circular label and `JournalAddActionButton` with a corner plus badge; Diet, WeightCalendar, and BodyTrend provide their own icon, action, state, and feature palette while the add-action geometry remains consistent.
- `ZheBuDeShouSi-iOS/ZheBuDeShouSi/Shared/UI/JournalCalendarStyle.swift`: shared Monday-first month-grid geometry, weekday rhythm, cell/surface tokens, selection accent, and empty-cell treatment used by the Diet and Weight journals.
...
- `ZheBuDeShouSi-iOS/ZheBuDeShouSi/Shared/UI/WeightCalendarView.swift`: shared `WeightModuleTitle`, local-date grouping, one representative weight per day, month grid presentation, and calendar intensity rules driven by the shared pink-to-blue journey palette; its grid and record action use the shared journal calendar and top-right circle-plus patterns.
...
- `ZheBuDeShouSi-iOS/ZheBuDeShouSi/Features/Diet/Application/DietStore.swift`: owns meal CRUD, strict in-place edits, confirmed-deletion persistence, local-day/month projections, kcal aggregation, image/payload limits, legacy meal-log migration, and HealthKit energy import with external-ID deduplication plus deletion tombstones. Production initialization is always empty; deterministic sample meals compile only inside the isolated `#if DEBUG` preview fixture. Views do not write `UserDefaults` directly.
...
- `ZheBuDeShouSi-iOS/ZheBuDeShouSi/Features/Diet/UI/DietUIModels.swift`: diet display mode, upload-state presentation, validated meal-edit draft, feature palette, and surface styling.
- `ZheBuDeShouSi-iOS/ZheBuDeShouSi/Features/Diet/UI/DietComponents.swift`: page header with the unified meal-add action, month calendar, photo mosaic, calorie overlays, compact in-content day upload action, day detail modal, single-open right-swipe action rows, centered meal editor, and review card.
- `ZheBuDeShouSi-iOS/ZheBuDeShouSi/Features/Diet/UI/DietView.swift`: coordinates month/mode/date selection, routes both upload actions into PhotosPicker loading, supports repeated multi-photo batches and review-first analysis, and owns edit/delete presentation state while all confirmed mutations go through `DietStore`.
...
- `DietView` in `Features/Diet/UI/DietView.swift`: the second tab is a dedicated food journal. Its header combines the former decorative cutlery mark and separate plus control into one top-right meal-add button. Its calendar shares the weight calendar's Monday-first grid, fixed rounded cells, spacing, weekday rhythm, selection treatment, and outer surface while supplying kcal-specific fills. It offers a month calendar and image mosaic, displays kcal per recorded day, opens a centered day detail modal whose compact upload row sits above the content, accepts up to six photos per batch and repeated batches without replacing earlier meals, and requires review before a meal enters the total. Meal rows have no trailing overflow button: a right swipe reveals one edit and one delete icon, only one row can remain open, edits use a centered validated form and retain the record ID/media/source, and deletion requires confirmation.
...
- Existing meal edits call `DietStore.updateMeal` rather than the insertion path. The method rejects missing IDs, preserves images and external metadata, and only clears food-level calorie estimates when the user clears the total. Confirmed deletion stores external sample IDs as optional version-two tombstones so HealthKit sync cannot recreate a record the user removed; version-one snapshots remain decodable.
...
- Weight calendar summaries are derived from local `Calendar.startOfDay` groups. Each day displays its latest record; changes compare with the previous day that has a record, not with the persisted `WeightRecord.change` field. `WeightJourneyPalette` is the single source for both the home rail and calendar: the starting weight maps to `jellyPink`, the configured goal maps to `jellyBlue`, and intermediate values use the identical interpolation regardless of numeric direction. Previous-day changes can strengthen a cell's fill but never shift its hue, so one weight always aligns with the same point on the rail. Values beyond the journey clamp to the nearest endpoint while heavier values retain stronger fill. `AppState.weightTone(_:)` remains unchanged for non-calendar weight numbers.

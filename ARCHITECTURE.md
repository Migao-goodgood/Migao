...
- `ZheBuDeShouSi-iOS/ZheBuDeShouSi/Features/BodyTrend/UI/BodyAvatarSceneRenderer.swift`: SceneKit-only renderer/controller for the parameterized Hello Kitty full-body avatar plus legacy human/animal/photo choices, materials, lighting, camera, interaction-friendly animation, and input-keyed scene updates; it has no persistence or domain mutations. Kitty uses a clean wall and depth-disabled contact shadow so no foreground prop can obscure the ears or feet.
...
- `BodyTrendView` in `Features/BodyTrend/UI/BodyTrendView.swift`: replaces the previous trend workspace with InBody photo/manual entry, a timeline-driven parameterized 3D avatar, mood and metric rows, and offline stage assessment. Hello Kitty is the default focal point and receives a taller, front-facing stage for an unobstructed ears-to-feet view; legacy character choices remain available from a compact menu and custom photo upload remains intact. Its visual language translates the supplied reference into paper-like spacing, hairline rules, restrained color accents, and asymmetric editorial layout without copying source imagery. The header keeps only the right-side scan/entry actions, while the empty state uses the available viewport height to center one visual composition rather than duplicating controls. Existing weight/body-measurement data remains owned by `AppState`/`HealthStore` and is not deleted.
